...
def read_friendship_network(filename):
    ...
    # init a variable to contain the friendship network
    network = {}
...
def add_friendship(network, name1, name2):
    ...
    # make sure no one can be added as their own friend
    if name1 == name2:
        return

    # if the dict already has data for name1, append to that
    # otherwise, create that key and add the first friendship in
    if name1 in network:
        # check if the friendship is already known
...
def print_command(network, namelist):
    ...
    # loop through each key in the dict in alphabetical order
    for person in sorted(network):
        print(person)
        # loop through each friend in the current person's
        # friendlist in alphabetical order
...
def friends_command(network, namelist):
    ...
    # check if the name is found network
    if name not in network:
        print(f"Error: '{name}' is an unknown name.")
        return

    # print a list of the persons friend in alphabetical order
    print("\n".join(sorted(network[name])))
...
def common_friends_command(network, namelist):
    ...
    # check that both of the names are found in the database
    for name in namelist:
        if name not in network:
            print(f"Error: '{name}' is an unknown name.")
            return
    ...
    # calculate the intersection (i.e common friends) of the two friend lists
    common_friends = set(name1_friends).intersection(name2_friends)

    if common_friends:
        # if common friends were found, print them in alphabetical order
        print("\n".join(sorted(common_friends)))
    else:
